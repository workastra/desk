...
describe('PaginatedResponse.toJSON', () => {
  ...
  it('should serialize correctly with empty data and meta', () => {
    const response = new PaginatedResponse('CODE001', [], sampleMeta);
  ...
  it('should serialize correctly with data array', () => {
    const data = [{ id: 1 }];
    const response = new PaginatedResponse('CODE002', data, sampleMeta);
    const json = response.toJSON();
  ...
  it('should throw if meta violates schema', () => {
    ...
    const response = new PaginatedResponse('CODE003', [], invalidMeta);
